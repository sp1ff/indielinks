...
indielinks itself is a workspace containing a number of crates:
  ...
  - tests-support :: support code for indielinks integration tests
  - tests-indielinks :: integration tests for the indielinks crate
  - tests-indielinks-cache :: integration tests for indielinks-cache
...
You may find it useful to generate rustdocs (especially the aforementioned documentation-only module):
...
This will stand-up a containerized three-node ScyllaDB cluster on the local machine & setup the schema.
...
#+BEGIN_SRC bash
  curl -v -H 'Content-Type: application/json' http://localhost:20680/ops/cache/init-cluster \
      --data "{\"nodes\": [[0, {\"addr\": \"127.0.0.1:20681\"}]], \"slots\": [[0,0],[1,0]]}" && \
  curl -v http://localhost:20679/healthcheck
#+END_SRC

This will configure the indielinks server as a single-node Raft cluster, and then healthcheck it. The healthcheck invocation should return status 202 and the response body should be the string "READY".
** Provision a User

Pick a username & password. Don't choose a trivial password: the signup endpoint requires a minimum complexity. Don't worry about the email address; indielinks doesn't validate it at this point.
...
Make a note of the API key that's printed on stdout.
** Start the Front End
...
Point your favorite browser to http://localhost:18080. You should be able to sign-in, add posts & view them. The "popular" page is in progress.
* Commit Procedure
