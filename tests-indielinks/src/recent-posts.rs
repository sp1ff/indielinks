...
use indielinks::{
    cache::{GrpcClient, SLOT_RECENT_POSTS, SLOT_TOP_K_TAGS},
    grpc::InitClusterRequest,
    recent_posts_lists::Client as RplClient,
};
...
///
/// Expects a cluster of at least three nodes. Now that we've moved the "raft ops" test into a
...
pub async fn recent_posts(
    ...
    let client = Client::builder()
        .user_agent("indielinks-test/recent-posts 0.0.1 (+sp1ff@pobox.com)")
        .build()?;
